		...
		<section className='mb-36 mt-24 mx-auto max-w-7xl px-6 2xl:px-0 '>
			...
			<div>
				...
				<div className='flex justify-between gap-4 flex-wrap'>
					{cities.map(city => (
						<article key={city.name} className='rounded-lg shadow-md bg-gray-200 relative grow transition-transform hover:scale-[1.01]'>
							<Link href={city.link} className='flex h-full flex-col'>
								<Image src={city.img} alt={city.name} className='object-cover grow rounded-md w-full max-h-96' width={200} height={400} loading='lazy' />
			...
			<div className='mt-16'>
				...
				<div className='flex justify-between gap-4 flex-wrap'>
					{filters.map(filter => (
						<article key={filter.name} className='rounded-lg shadow-md bg-gray-200 relative grow transition-transform hover:scale-[1.01]'>
							<Link href={filter.link} className='flex h-full flex-col'>
								<Image src={filter.img} alt={filter.name} className='object-cover grow rounded-md w-full max-h-96' width={200} height={400} loading='lazy' />
